    ...
    <div class="py-12 bg-platinum min-h-screen font-titillium">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                <div>
                    <h3 class="text-2xl font-bold text-midnight">Direktori Pegawai</h3>
                    <p class="text-slate-500 text-sm">Kelola data pegawai dan cetak kartu presensi QR Code.</p>
                </div>
                <a href="{{ route('pegawai.create') }}" class="group relative inline-flex items-center justify-center px-6 py-3 font-bold text-midnight transition-all duration-200 bg-gold rounded-lg hover:bg-yellow-400 hover:shadow-[0_0_20px_rgba(238,191,99,0.5)] focus:ring-2 ring-gold">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden mb-10">
                ...
                {{-- UPDATE: Header Panel menggunakan bg-midnight (Biru Gelap) --}}
                <div class="bg-midnight px-6 py-4 border-b-4 border-gold flex justify-between items-center relative overflow-hidden">
                    ...
                    <div class="relative z-10 flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-bold text-white tracking-wide">Daftar Pegawai Aktif</h3>
                    </div>
                    <div class="relative z-10">
                        <span class="bg-white/10 text-gold text-xs font-bold px-3 py-1 rounded-full border border-gold/30 backdrop-blur-sm">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full text-left">
                        {{-- UPDATE: Header Kolom Tabel menggunakan bg-midnight --}}
                        <thead class="bg-midnight text-white uppercase text-xs tracking-wider">
                            <tr>
                                <th class="py-4 px-6 font-bold">Identitas Pegawai</th>
                                <th class="py-4 px-6 font-bold">Jabatan</th>
                                <th class="py-4 px-6 font-bold">Divisi</th>
                        ...
                        <tbody class="divide-y divide-slate-100 text-sm bg-white">
                                ...
                                <tr class="hover:bg-slate-50 transition duration-150 group">
                                    ...
                                    <td class="py-4 px-6">
                                        <div class="flex items-center">
                                            <div class="h-10 w-10 rounded-full bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center text-white font-bold shadow-md mr-4 ring-2 ring-white group-hover:ring-indigo-200 transition">
                                                {{ substr($p->nama, 0, 1) }}
                                            </div>
                                            <div>
                                                <div class="font-bold text-midnight text-base group-hover:text-indigo-600 transition">{{ $p->nama }}</div>
                                                <div class="text-xs text-slate-400 font-mono mt-0.5 tracking-wide bg-slate-100 px-2 py-0.5 rounded inline-block">NIP: {{ $p->nip }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        <div class="flex justify-center items-center gap-2">
                                            ...
                                            {{-- Hapus --}}
                                            <form action="{{ route('pegawai.destroy', $p->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data pegawai ini?');">
                                                @csrf @method('DELETE')
                            ...
                            @empty
                                <tr><td colspan="4" class="py-12 text-center text-slate-400 bg-slate-50/50">Belum ada data pegawai.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
